...
public class RegistryUtil {
    ...
    public static int getRegistoryInt(int hKeyParent, String subKeyString, String valueString) {
    ...
    public static String getRegistoryString(int hKeyParent, String subKeyString, String valueString) {
        int hKey = open(hKeyParent, subKeyString, 0, KEY_READ);
        if( 0 != hKey ) {
            try {
    ...
    private static int open(int hKeyParent, String subKeyString, int ulOptions, int samDesired) {
        ...
        int[] phKey = new int[1];
        if (0 == OS.RegOpenKeyEx(hKeyParent, keyName, ulOptions, samDesired, phKey)) {
            return phKey[0];
        }
        return 0;
    }
    ...
    private static String getStringValue(int hKey, String valueString) {
        ...
        int[] pType = new int[1];
        if (0 == OS.RegQueryValueEx(hKey, valueName, 0, pType, (TCHAR)null, pcbData)) {
            if( REG_SZ == pType[0] ) {
                TCHAR buff = new TCHAR(0,pcbData[0]/TCHAR.sizeof);
                if (0 == OS.RegQueryValueEx(hKey, valueName, 0, pType, buff, pcbData)) {
