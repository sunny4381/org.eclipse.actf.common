...
public class WebBrowserIEComposite extends Composite implements IWebBrowserIEConstants {
    ...
    // Event IDs
    // TODO in capital
    private static final int EventID_BEFORENAVIGATE = 100, EventID_NAVIGATECOMPLETE = 101,
            EventID_STATUSTEXTCHANGE = 102, EventID_QUIT = 103, EventID_DOWNLOADCOMPLETE = 104,
            EventID_COMMANDSTATECHANGE = 105, EventID_DOWNLOADBEGIN = 106, EventID_NEWWINDOW = 107,
            EventID_PROGRESSCHANGE = 108, EventID_WINDOWMOVE = 109, EventID_WINDOWRESIZE = 110,
            EventID_WINDOWACTIVATE = 111, EventID_PROPERTYCHANGE = 112, EventID_TITLECHANGE = 113,
            EventID_TITLEICONCHANGE = 114, EventID_FRAMEBEFORENAVIGATE = 200, EventID_FRAMENAVIGATECOMPLETE = 201,
            EventID_FRAMENEWWINDOW = 204, EventID_BEFORENAVIGATE2 = 250, EventID_NEWWINDOW2 = 251,
            EventID_NAVIGATECOMPLETE2 = 252, EventID_ONQUIT = 253, EventID_ONVISIBLE = 254, EventID_ONTOOLBAR = 255,
            EventID_ONMENUBAR = 256, EventID_ONSTATUSBAR = 257, EventID_ONFULLSCREEN = 258,
            EventID_DOCUMENTCOMPLETE = 259, EventID_ONTHEATERMODE = 260, EventID_ONADDRESSBAR = 261,
            EventID_WINDOWSETRESIZABLE = 262, EventID_WINDOWCLOSING = 263, EventID_WINDOWSETLEFT = 264,
            EventID_WINDOWSETTOP = 265, EventID_WINDOWSETWIDTH = 266, EventID_WINDOWSETHEIGHT = 267,
            EventID_CLIENTTOHOSTWINDOW = 268, EventID_SETSECURELOCKICON = 269, EventID_FILEDOWNLOAD = 270,
            EventID_NAVIGATEERROR = 271, EventID_PRIVACYIMPACTEDSTATECHANGE = 272, EventID_NEWWINDOW3 = 273;
    ...
    public WebBrowserIEComposite(Composite parent, int style) {
        ...
        try {
            ...
            // site2 = new OleControlSite(frame, SWT.NONE, BROWSER_PROGID);
            // site2.setVisible(false);
            // auto2 = new OleAutomation(site2);
                ...
                if (null != auto) {
                    try {
                        ...
                        case EventID_BEFORENAVIGATE2:
                            onBeforeNavigate2(new BeforeNavigate2ParametersImpl(event));
                            break;
                        case EventID_NAVIGATECOMPLETE2:
                            onNavigateComplete2(new NavigateComplete2ParametersImpl(event));
                            break;
                        case EventID_NAVIGATEERROR:
                            onNavigateError(new NavigateErrorParametersImpl(event));
                            break;
                        case EventID_DOCUMENTCOMPLETE:
                            onDocumentComplete(new DocumentCompleteParametersImpl(event));
                            break;
                        case EventID_NEWWINDOW2:
                            onNewWindow2(new NewWindow2ParametersImpl(event));
                            break;
                        case EventID_WINDOWCLOSING:
                            onWindowClosing(new WindowClosingParametersImpl(event));
                            break;
                        case EventID_STATUSTEXTCHANGE:
                            onStatusTextChange(new StatusTextChangeParametersImpl(event));
                            break;
                        case EventID_TITLECHANGE:
                            onTitleChange(new TitleChangeParametersImpl(event));
                            break;
                        case EventID_PROGRESSCHANGE:
                            onProgressChange(new ProgressChangeParametersImpl(event));
                            break;
                        default:
                            System.out
        ...
        // OleListener subBrowserListener = new OleListener() {
        // public void handleEvent(OleEvent event) {
        // if (null != auto2) {
        // try {
        // switch (event.type) {
        // case EventID_BEFORENAVIGATE2:
        // subBrowser_onBeforeNavigate2(new BeforeNavigate2ParametersImpl(event));
        // break;
        // }
        // }
        // catch (Exception e) {
        ...
        // };
        //        
        // site2.addEventListener(EventID_BEFORENAVIGATE2, subBrowserListener);
    }
    ...
    public boolean navigate(String url) {
        int[] pIdNavigate = auto.getIDsOfNames(new String[] { "Navigate", "URL" }); //$NON-NLS-1$ //$NON-NLS-2$
        if (null != pIdNavigate) {
            Variant varUrl = new Variant(url);
            Variant varResult = auto.invoke(pIdNavigate[0], new Variant[] { varUrl }, new int[] { pIdNavigate[1] });
            varUrl.dispose();
            if (varResult != null) {
    ...
    public boolean navigate2(String url, int flags, String targetFrameName, Object postData, String headers) {
        ...
        if (null != pIdNavigate2) {
            ...
            Variant varTargetFrameName = null == targetFrameName ? new Variant() : new Variant(targetFrameName);
            Variant varHeaders = null == headers ? new Variant() : new Variant(headers);
            if (!(postData instanceof Variant)) {
                postData = new Variant();
            }
            Variant varResult = auto.invoke(pIdNavigate2[0], new Variant[] { varUrl, varFlags, varTargetFrameName,
                    (Variant) postData, varHeaders });
            varUrl.dispose();
            varFlags.dispose();
    ...
    public boolean setFontSize(int nSize) {
        ...
        try {
            return OLE.S_OK == site.exec(OLE.OLECMDID_ZOOM, OLE.OLECMDEXECOPT_DONTPROMPTUSER, varIn, null);
        } catch (Exception e) {
        } finally {
            varIn.dispose();
        }
    ...
    public int getFontSize() {
        ...
        try {
            if (OLE.S_OK == site.exec(OLE.OLECMDID_ZOOM, OLE.OLECMDEXECOPT_DONTPROMPTUSER, null, varOut)) {
    ...
    protected void onNewWindow2(NewWindow2Parameters param) {
        // int[] pIdState = auto2.getIDsOfNames(new String[] {"ReadyState"}); //$NON-NLS-1$
        // if (pIdState != null) {
        // System.out.println("Browser2 readyState="+auto.getProperty(pIdState[0])); //$NON-NLS-1$
    ...
    // */
    // protected void subBrowser_onBeforeNavigate2(BeforeNavigate2Parameters param) {
    ...
    private void setEventListeners(Set set) {
        eventListeners = (BrowserEventListener[]) set.toArray(new BrowserEventListener[set.size()]);
    }
    ...
    private static boolean saveDocument(Variant varDocument, String fileName) {
        if (OLE.VT_DISPATCH == varDocument.getType()) {
            ...
            try {
                if (OLE.S_OK == varDocument.getDispatch().QueryInterface(COM.IIDIPersistFile, pPersistFile)) {
                    int pStrAddress = COM.SysAllocString((fileName + "\0").toCharArray()); //$NON-NLS-1$
                    try {
                        return OLE.S_OK == new IPersistFile(pPersistFile[0]).Save(pStrAddress, false);
                    } finally {
    ...
    int siteWindowProc(int hwnd, int msg, int wParam, int lParam) {
        try {
            if (!isClosed && OS.WM_PARENTNOTIFY == msg && wParam == OS.WM_DESTROY) {
                isClosed = true;
                onWindowClosed();
                return 0;
            }
