...
public abstract class HTTPMessageReader extends HTTPReader {
    ...
    protected void processHeader(HTTPMessage msg, HTTPHeader header) {
    }
    ...
    public boolean readMessage(HTTPMessage msg, long timeout, boolean isBodyEmpty)
        ...
        while (fLastByte >= 0) {
            ...
            if (header.isFieldNameEqualsTo(HTTPHeader.CONTENT_LENGTH_A)) {
            ...
            } else if (header.isFieldNameEqualsTo(HTTPHeader.TRANSFER_ENCODING_A) 
                       && header.isFieldValueEqualsTo(HTTPHeader.CHUNKED_A)) {
